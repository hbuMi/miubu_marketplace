...
def get_item(item_id):
    sql = "SELECT items.id, items.title, items.descr, items.price, items.section_id, items.condition_id, users.id user_id, users.username FROM items JOIN users ON items.user_id = users.id WHERE items.id = ?"
